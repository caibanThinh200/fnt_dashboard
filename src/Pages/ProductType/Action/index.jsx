...
const ProductTypeAction = (props) => {
    ...
    useEffect(() => {
        if (attributes.result.length > 0) {
            ...
            ;(attributes.result || []).map((attribute) => {
                if (
                    ...
                    !listAttributesIds.attributes.includes(attribute?.id)
                ) {
                    listAttributesIds = {
                        attributes: [
                            ...listAttributesIds.attributes,
                            attribute?.id,
                        ],
                    }
        ...
        if (Object.keys(props.productTypes).length > 0) {
            ...
            if (
                (props.productTypes.item?.attributes || []).length > 0 &&
                attributes.result.length > 0
            ) {
                let listAttributesIds = {
                        attributes:
                            (props.productTypes.item?.attributes || []).map(
                                (item) => item?.id
                            ) || [],
                    },
            ...
            if (
                Object.keys(props.productTypes.item?.filter || {}).length > 0 &&
                attributes.result.length > 0
            ) {
                Object.keys(props.productTypes.item?.filter || {}).map(
                    (item) => {
                        form.setFieldsValue({
                            [`filter.${item}.value`]:
                                props.productTypes.item?.filter[item]?.value ||
                                [],
                        })
                    }
                )
    ...
    const handleSubmit = (e) => {
        ...
        Object.keys(e).map((item) => {
            if (item.includes('filter')) {
                ...
                filterParams = {
                    ...props.productTypes.item?.filter,
                    ...filterParams,
                }
    ...
    const handleAttributeDeselect = (value) => {
        ...
        if (currentAttribute?.required_field) {
            ...
            message.warning(
                'Thông số này không thể thiếu trong loại sản phẩm ' +
    ...
    const actionTitle = `${action} ${lowercaseFirstLetter(
        TEXT_DEFINE.PAGE.PRODUCT_TYPE.title
    )}`
                            ...
                            {filter.length > 0 &&
                                filter.map((item) => (
                                    <Collapse className="mb-3">
                                        <Collapse.Panel
                                            header={`Bộ lọc của trường ${item.name}`}
                                        >
                                            <FormList
                                                label={
                                                    TEXT_DEFINE.PAGE
